...
def get_network_configuration(analysis_type, noconns=False):
    ...
    # ADD THIS LINE:
    Iexts = np.array([6, 0, 8, 0])  # Default baseline input
...
def calculate_periodogram(re, transient, dt):
    ...
    Inputs:
        re: Signal to be analysed (1D array)
        transient: Time to discard at start (in seconds)
    ...
    restate = re[transient_idx:]
    
    # Get length of signal after removing transient
    N = len(restate)
    ...
    # Perform periodogram with window as STRING (not array!)
    # This avoids the size mismatch error
    fxx, pxx = signal.periodogram(restate, fs=fs, 
                                  window='boxcar',  # ← String, not array!
                                  nfft=fft, 
                                  detrend=False, 
